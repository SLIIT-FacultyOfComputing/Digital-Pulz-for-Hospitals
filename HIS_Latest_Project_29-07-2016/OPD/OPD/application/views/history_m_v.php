...
<!-- Message for operation status  ************************************************************** --> 
		...
		<div class="alert alert-success">
			...
			<strong>Successful!  </strong> Record Added Successfully
		</div>
		<?php }else if(  preg_match('/Edit/',$title)  &  $status == "True"){  ?>
		
		<div class="alert alert-success">
			<button type="button" class="close" data-dismiss="alert">&times;</button>
			<strong>Successful!  </strong> Record updated Successfully
		</div>
		...
		<?php if((!preg_match('/Edit/',$title)) &  $status == "False"){ ?>
			...
			<div class="alert alert-danger">
				<button type="button" class="close" data-dismiss="alert">&times;</button>
				<strong>Fail !</strong> Failed to add the Record
			</div>
		...
		<?php }else if(  preg_match('/Edit/',$title) &  $status == "False"){  ?>
			...
			<div class="alert alert-danger">
				<button type="button" class="close" data-dismiss="alert">&times;</button>
				<strong>Fail !</strong> Failed to update the Record
			</div>
		<?php } ?>
	<?php } ?>
 <!-- **************************************************************************************** --> 		
 ...
 <div class="form-group input-group" style="margin-top: 10px">
     ...
     <textarea type="text" class="form-control" style="min-width: 200px;max-width: 200px;min-height: 60px;height:auto" id="RecordText"  required name="RecordText" placeholder=""><?php if (preg_match('/Edit/', $title)) echo $record[0]->recordText; ?></textarea>
 ...
 <div class="container-fluid">
     <div class="form-actions">
         ...
         <button type="reset" class='btn' onclick="document.location.href='<?php if ($this->session->userdata ( 'userlevel' ) == 1) {echo site_url ( '/patient_overview_c/view/' . $pid );} else {echo site_url ( 'operator_home_c/view/1' );} ?>'">Back</button>
         &nbsp;&nbsp;&nbsp;&nbsp;
     </div>
 </div>
