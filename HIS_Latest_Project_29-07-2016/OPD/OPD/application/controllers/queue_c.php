...
session_start();
    class queue_c  extends CI_Controller{
			...
			public function add($pid,$status = '0',$full=null, $available='true')
			{
...
//                                        die;
			...
			if (isset($_SESSION["user"])) {
	            ...
	            if($formSubmit == 'auto')
                {
                    $data = json_decode($this->queue->addToQueueAuto($visitType), true);
                }else {
                    $data = json_decode($this->queue->addToQueue(), true);
                }

                if(isset($data['available']))
				    $this->add($pid,$data['status'],$data['full'], $data['available']);
	            else
			...
			{ 
				...
				$doctor = $this->session->userdata("userid");
 				$this->queue->redirectQueue($doctor,$this->queue->getQTypeForDoctor($doctor));
				$url = base_url()."index.php/doctor_home_c/view/5";
				header("Location:".$url);
			}
