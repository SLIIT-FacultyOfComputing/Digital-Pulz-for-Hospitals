...
class QueueModel {
    ...
    public function addToQueueAuto($visittype)
    {
        $queueJson   = json_encode($this->jsonSerialize());
        $service_url = SERVICE_BASE_URL."Queue/addPatientToQueueAuto/".$visittype;
        $MediaType = "application/json";
        $curl_request  = new ServiceCaller();
        $response =  $curl_request->curl_POST_Request($service_url,$queueJson,$MediaType);
        return $response;
	...
	public function redirectQueue($doctor, $type)
	{
		$service_url = SERVICE_BASE_URL."Queue/redirectQueue/".$doctor."/".$type;
		$curl_request = new ServiceCaller();
		$response = $curl_request->curl_GET_Request($service_url);
		return $response;
	} 
